...
class PigeonContainerTest < UnitTest
  ...
  def test_missing_key
    ex = assert_raises(Pigeon::Container::KeyNotFound) do
      Pigeon::Container.new.resolve('hello')
    end
  ...
  def test_duplicate_key
    container = Pigeon::Container.new
    container.register('cat') { 'meow' }
    ex = assert_raises(Pigeon::Container::DuplicateKey) do
  ...
  def test_cyclic_dependencies
    ...
    ex = assert_raises(Pigeon::Container::CyclicDependencies) do
      container['cheese']
    end
  ...
  def test_missing_block
    ex = assert_raises(ArgumentError) do
      Pigeon::Container.new.register('x')
    end
  ...
  def test_bad_option
    ex = assert_raises(ArgumentError) do
      Pigeon::Container.new.register('x', bad_option: 'moo', bad2: true) { 'x' }
    end
